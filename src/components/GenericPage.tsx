    ...
    <>
      <section className="relative overflow-hidden bg-slate-950 px-4 py-16 text-white sm:py-20">
        <img
          ...
          aria-hidden="true"
        />
        <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(2,6,23,0.88)_0%,rgba(15,23,42,0.74)_46%,rgba(30,64,175,0.36)_76%,rgba(2,6,23,0.16)_100%)]" />
        ...
        <div className="relative mx-auto max-w-7xl lg:px-6">
          <div className="max-w-4xl">
            <p className="inline-flex items-center gap-2 rounded-md bg-white/15 px-3 py-1.5 text-sm font-black uppercase text-white ring-1 ring-white/25 backdrop-blur">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              ...
              <Link href="/#kontakt" className="inline-flex items-center gap-2 rounded-lg border border-white/30 bg-white/10 px-5 py-3 font-black text-white backdrop-blur transition hover:bg-white/20">
                Nezáväzná ponuka
                <ArrowRight size={18} />
              </Link>
            </div>
      ...
      {isArticle && page.images[0] ? (
        <section className="bg-white px-4 pt-12">
          <div className="mx-auto max-w-5xl">
            <img src={page.images[0].src} alt={page.images[0].alt || page.h1} className="max-h-[460px] w-full rounded-lg object-cover" />
          </div>
        </section>
      ) : null}
        ...
        <section className="bg-blue-50 px-4 py-12">
          <div className="mx-auto flex max-w-7xl flex-col gap-5 lg:flex-row lg:items-center lg:justify-between lg:px-6">
            <div className="flex items-start gap-3">
              <ShieldCheck size={28} className="mt-1 shrink-0 text-blue-700" />
              <div>
            ...
            <Link href="/#kontakt" className="inline-flex w-fit rounded-lg bg-blue-600 px-5 py-3 font-black text-white transition hover:bg-blue-700">
              Kontaktovať JP TRANSPORT
            </Link>
          </div>
        </section>
      ) : null}
